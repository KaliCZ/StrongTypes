    ...
    where TInterval : struct
{
    ...
    // ── Invalid payloads ─────────────────────────────────────────────────
    // The interval converter throws JsonException while positioned on the
    // property, so System.Text.Json keys the error by its path ("$.value" /
    // "$.nullableValue"). The Api harness wires no key normalization (it does
    // not call AddStrongTypes), so these are the raw framework paths — the
    // contract codified by #106.
