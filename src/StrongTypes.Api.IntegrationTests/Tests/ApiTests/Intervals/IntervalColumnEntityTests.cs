    ...
    where TInterval : struct
{
    ...
    public void ValueMapsToTwoSeparateEndpointColumns()
    {
        ...
        Assert.Contains(nameof(ClosedInterval<int>.Start), endpointColumns);
        Assert.Contains(nameof(ClosedInterval<int>.End), endpointColumns);
...
public sealed class ClosedIntervalColumnEntityTests(TestWebApplicationFactory factory)
    : IntervalColumnEntityTestsBase<ClosedIntervalColumnsEntity, ClosedInterval<int>>(factory)
{
...
{
    protected override Interval<int> ValidValue => Interval<int>.Create(1, 10);
}
...
{
    protected override IntervalFrom<int> ValidValue => IntervalFrom<int>.Create(1, null);
}
...
{
    protected override IntervalUntil<int> ValidValue => IntervalUntil<int>.Create(null, 10);
}
