...
/// <remarks>Works for any <typeparamref name="T"/> with a public static <c>TryParse(string, IFormatProvider?, out T)</c> — every BCL primitive in net7+ and every Kalicz.StrongTypes wrapper.</remarks>
internal static class StringElementParser<T>
{
    ...
    /// <summary>True when <typeparamref name="T"/> exposes the <c>IParsable</c> shape.</summary>
