...
- **Test base class** — inherit from `IntegrationTestBase(factory)` to get everything: `Client`, `SqlDb`, `PgDb`, `Ct` (current test's CancellationToken), route constants/builders, HTTP wrappers (`Post`/`Put`/`Get`) that capture `Client` and `Ct` implicitly and bake in `StringEntityResponse` on the success path, `Body<T>(value, nullableValue)`, and `AssertStringEntity`. A follow-up PR will make this base generic over the entity type.
