...
| `FiniteInterval<T>` / `Interval<T>` / `IntervalFrom<T>` / `IntervalUntil<T>` | `{ "type": "object", "properties": { "Start": <T schema>, "End": <T schema>, "StartInclusive": bool default true, "EndInclusive": bool default true }, "required": ["Start", "End"] }` — a bounded endpoint is non-nullable, an unbounded one nullable; only the variant's bounded endpoints are required |
